  ...
  const getInventory = (storeId) => {
    return new Promise((resolve, reject) => {
      connPool.getConnection((err, connection) => {
        ...
        connection.query(`SELECT store_name, store_id FROM USERS, STORES WHERE STORES.store_id = ?;`, [storeId], (error, result) => {
          if(error){
            response.statusCode = 418
            response.body = error.message
            return resolve(error)
        ...
        connection.query(`SELECT * FROM DEVICES WHERE DEVICES.store_id = ?;`, [storeId], (error, result) => {
          console.log("HERE")
          if (error) {
            console.log(error)
            response.statusCode = 418
          ...
          connection.query(`SELECT SUM(price) as balance FROM DEVICES, USERS, STORES WHERE DEVICES.store_id = ?;`, [storeId], (error, result) => {
            console.log("HERE")
            if (error) {
              console.log(error)
              response.statusCode = 418
