...
import { Link, MetaFunction, Outlet, useLoaderData, useNavigate } from "@remix-run/react";
...
export async function loader({ request }: LoaderFunctionArgs) {
  return json({apiBaseUrl: process.env.API_BASE_URL, isAuthenticated: await authenticator.isAuthenticated(request) ? true: false, });
};
  ...
  return (
    <div className="p-4">
      ...
      {isAuthenticated && (
        ...
        <button className="flex items-center rounded-md border-2 px-4 text-center" onClick={() => navigate("/auth/callback")}>{isAuthenticated ? "Log Out": "Log In"}</button>
      </div>
        <Outlet />
      </QueryClientProvider>
    </div>
